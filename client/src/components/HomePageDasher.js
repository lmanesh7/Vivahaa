...
import { Grid, Card, CardContent, CardMedia, Typography, CardActionArea, FormControl, InputLabel, Select, MenuItem, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, Button } from '@mui/material';
...
const HomePageDasher = () => {
  ...
  const [filterValue, setFilterValue] = useState(localStorage.getItem('location')?localStorage.getItem('location'):'');
    ...
    const fetchData = async () => {
      try {
        if (!dataFetched) {
          ...
          const mehendiArtistData =
            await MehendiArtistsService.getAllMehendiArtists();
  ...
  venue.address.toLowerCase().includes(filterValue.toLowerCase())
);

const filteredMehendi = mehendiArtists.filter((venue) =>
venue.address.toLowerCase().includes(filterValue.toLowerCase())
);


const filteredDecor = decorators.filter((venue) =>
venue.address.toLowerCase().includes(filterValue.toLowerCase())
);

const filteredPhoto = photographers.filter((venue) =>
venue.address.toLowerCase().includes(filterValue.toLowerCase())
);



  const venuesList = filteredVenues.map((v) => {
    return {
      _id: v._id,
  ...
  const mehendiArtistsList = filteredMehendi.map((m) => {
    return {
      _id: m._id,
  ...
  const decoratorsList = filteredDecor.map((d) => {
    return {
      _id: d._id,
  ...
  const photographerList = filteredPhoto.map((p) => {
    return {
      _id: p._id,
  ...
  const gridItems_photographers = createGridItems_Photographer([
    ...photographerList,
  ]);
